...
export const AuthGuard = ({
	...
	const { loginUserPermissions, metaDataLoading } = useAppSelector(
		(state) => state.user
	);
	...
	if (loginUserPermissions.length > 0) {
		const modulePermission = loginUserPermissions.find(
			(permission) => permission.module.name.toLowerCase() === module
		);

		if (modulePermission && !modulePermission.module.isActive) {
