...
import { fetchPaymentTerms } from "../../store/Reducers/CustomerReducers";
...
const AddCustomer = () => {
	...
	const { paymentTerms } = useAppSelector((state) => state.customer);
			...
			>
				...
				>
					...
					<ModulePage
						...
						editCallBack={(e: any) => {
							const paymentTerm = paymentTerms.find((term) => term.id === e[0]);
				...
				>
					...
					>
						<Typography variant="subtitle1" sx={{ width: "30%" }}>
							Customer Code
							<CustomTooltip text="Leave it empty to generate automatically" />
						</Typography>
						<Controller
							name="customerCode"
							control={control}
							...
							render={({ field }) => (
								<TextField
									...
									slotProps={{
										input: {
											startAdornment: (
												<InputAdornment position="start">CUS</InputAdornment>
											),
										},
									}}
									error={Boolean(errors.customerName)}
