...
import RenderListViewCell from "../../components/RenderListViewCell";
...
const roleListViewRow = (role: Role) => {
  return {
    ...
    isSystemAdded: role.isSystemAdded,
          ...
          <Box key={index}>
            {sections.map((menu) => (
              ...
              >
                <MenuItem
                  key={menu.name}
                  onClick={() => menu.action && menu.action(e.row.id as number)}
                >
                  <Box
                    sx={{
                      display: "flex",
...
const RenderRoleMoreIcon = (e: GridRenderCellParams) => {
  ...
  const modulePermissions = loginUserPermissions.find(
    (module) => module.module.name.toLowerCase() === "roles"
  );
    ...
    [
      {
        ...
        icon: <EditOutlinedIcon sx={{ fontSize: "20px" }} />,
        color: "primary.main",
        isHidden: !modulePermissions?.canUpdate,
        isDisabled: e.row.isSystemAdded,
        tooltip: e.row.isSystemAdded
          ? "System added roles cannot be edited"
          : "",
      },
      {
        key: "delete",
        ...
        icon: <DeleteOutlineOutlinedIcon sx={{ fontSize: "20px" }} />,
        color: "error.main",
        isHidden: e.row.isSystemAdded || !modulePermissions?.canDelete,
      },
    ],
  ];
