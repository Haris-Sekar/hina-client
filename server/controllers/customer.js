...
export const getCustomers = async (req, res) => {
	...
	try {
		...
		if (customerId != null) {
			const customerDetails = await Customer.getCustomerDetails(
				companyId,
				customerId
			);
			respCode = 200;
			...
			const customerDetails = await Customer.getCustomers(
				companyId,
				index,
				range
			);
...
export const updateCustomer = async (req, res) => {
	...
	updateCustomerTry: try {
		...
		if (
			...
			response.message =
				"customerId and the above mentioned params should be present";
			respCode = 400;
			breakFlag = 1;
		}
...
export const deleteCustomers = async (req, res) => {
	...
	deleteCustomerTry: try {
		...
		if (!ids && ids.length === 0) {
			...
			response = {
				message: "Field validation error",
				fields: "customerIds should be provided",
			};
			break deleteCustomerTry;
		}
	...
	addMainAreaTry: try {
		...
		if (result.affectedRows > 0) {
			...
			response = {
				...
				code: 201,
				mainArea: await MainArea.getMainArea(req.companyId, result.insertId),
			};
			respCode = 201;
		}
	} catch (e) {
...
export const getMainArea = async (req, res) => {
	...
	try {
		...
		if (mainAreaId != null) {
			...
			const areaDetails = await MainArea.getMainAreas(
				companyId,
				undefined,
				undefined
			);
...
export const updateMainArea = async (req, res) => {
	...
	updateMainAreaTry: try {
		...
		const result = await MainArea.updateMainArea(
			{ name },
