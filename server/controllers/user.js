...
export const signup = async (req, res) => {
	...
	try {
		...
		const newUser = new Users(name, email, mobile, "", false);
...
export const accountVerify = async (req, res) => {
	...
	try {
		...
		if (tokenData.dateCreated <= new Date().getTime()) {
			const result = await userObj.updateUser(
				{ is_verified: true },
				`email = "${userObj.email}"`
			);
			response = {
...
export const getUserAndCompanyDetails = async (req, res) => {
	let code, response;

	try {
		const userDetails = await Users.getUserDetails(req.userId);
		const companyDetails = (await Company.getUserCompanies(req.userId))[0];
		code = 200;
		response = {
			userDetails,
			companyDetails,
			code: 200,
			message: "User and company details fetched successfully",
		};
	} catch (error) {
		code = 500;
		response = {
